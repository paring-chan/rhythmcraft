...
app.get('/remove-report', utils.isAdmin, async (req, res, next) => {
  ...
  await Chat.updateOne(
    { chat_id: req.query.chat, reported: true },
    { reported: false, reported_by: 'no_user' },
  )
...
app.post('/admin_create_promotion', utils.isAdmin, async (req, res, next) => {
  ...
  if (req.body.type == 'item' && !req.body.promotion_item) {
    req.flash(
      'Error',
      '프로모션 코드 타입이 아이템일 경우 아이템 코드를 입력해야 합니다!',
    )
    return res.redirect('/admin/create-promotion')
  }
  ...
  if (req.body.multi_code == 'true' && req.body.count < 2) {
    req.flash(
      'Error',
      '코드 여러개 생성시 코드 생성 갯수는 2 이상이어야 합니다!',
    )
    return res.redirect('/admin/create-promotion')
  }
  ...
  if (req.body.type == 'item') {
    const checkitem = await Item.findOne({
      product_id: req.body.promotion_item,
    })
    if (!checkitem) {
  ...
  if (req.body.multi_code == 'true') {
    ...
    req.flash(
      'Info',
      `프로모션 코드가 생성되었습니다!<br>코드 : ${promotion_code}`,
    )
    return res.redirect('/admin/create-promotion')
  }
})
...
app.post('/promotion-api', async (req, res, next) => {
  if (req.body.apikey != setting.API_SECRET)
    return res.json({ error: true, message: 'API 키가 틀렸습니다.' })

  const allowed_type = ['money', 'item']
  if (!allowed_type.includes(req.body.type)) {
    return res.json({
      error: true,
      message: '프로모션 코드 타입이 잘못되었습니다!',
    })
  }
  if (req.body.type == 'money' && !req.body.promotion_money) {
    return res.json({
      error: true,
      message: '프로모션 코드 타입이 돈일 경우 돈을 입력해야 합니다!',
    })
  }
  if (req.body.type == 'item' && !req.body.promotion_item) {
    return res.json({
  ...
  if (req.body.type == 'item') {
    ...
    if (!checkitem) {
      return res.json({
        error: true,
        message: '해당 아이템 코드로 아이템을 찾을 수 없습니다!',
      })
    }
  }
