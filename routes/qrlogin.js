...
app.post('/qrloginapi', (req, res, next) => {
  ...
  if (!beta_mode && req.body.alphaapp == 'true')
    return req.app.get('socket_qrlogin').to(req.body.socketID).emit('msg', {
      action: 'alert',
      msg:
        '알파, 베타 테스트중이 아닙니다. 프로덕션 앱을 구글 플레이에서 다운받으세요.',
    })
  if (deny_appver.indexOf(req.body.appversion) != -1)
    return req.app.get('socket_qrlogin').to(req.body.socketID).emit('msg', {
      action: 'alert',
...
app.get('/getqrcode', utils.isNotLogin, async (req, res, next) => {
  ...
  const data = {
    RequestURL: `${setting.SITE_BASEURL}/qrloginapi`,
...
app.get('/qrappprivacy', (req, res, next) => {
  return res.send(
    `<h1>카메라는 QR코드 인식에만 사용되며, 이미지는 저장되지 않습니다.</h1>`,
  )
})
