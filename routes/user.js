...
app.post('/editaccount', utils.isLogin, async (req, res, next) => {
  ...
  if (req.body.nickname.includes('<') || req.body.nickname.includes('>')) {
    req.flash(
      'Error',
      '닉네임에 허용되지 않은 문자 < 또는 >가 포함되어 있습니다!',
    )
    return res.redirect('/mypage')
  }
...
app.get('/profile', async (req, res, next) => {
  const profile_user = await User.findOne({
    fullID: req.query.id || req.user.fullID,
  })
  if (!profile_user) {
  ...
  let profile_image = await File.findOne({
    owner: req.query.id || req.user.fullID,
    file_type: 'avatar',
  })
  ...
  let badge
  if (profile_user.equip.image_badge != null)
    badge = await Item.findOne({ product_id: profile_user.equip.image_badge })
...
app.post('/friend_request', utils.isLogin, async (req, res, next) => {
  ...
  if (friend_user.friend_request.includes(req.user.fullID)) {
    req.flash(
      'Error',
      '해당 유저에게 이미 친구 요청을 보냈습니다. 응답을 기다려주세요!',
    )
    return res.redirect('/friend')
  }
...
app.get(
  ...
  async (req, res, next) => {
    ...
    req.flash(
      'Info',
      `친구 요청을 받아들여 ${target_friend.nickname}님과 친구가 되었습니다!`,
    )
    return res.redirect('/friend')
  },
)
...
app.get('/remove_friend/:user', utils.isLogin, async (req, res, next) => {
  ...
  req.flash(
    'Info',
    `${
      target_friend != null ? target_friend.nickname : '알 수 없음'
    }님을 친구 목록에서 삭제하였습니다.`,
  )
  return res.redirect('/friend')
})
