...
app.get('/editor', utils.isLogin, async (req, res, next) => {
  ...
  const note_file = fs
    .readFileSync(path.join(setting.SAVE_FILE_PATH, note.name))
    .toString()
...
app.get('/testnote', utils.isLogin, async (req, res, next) => {
  ...
  const music = await File.findOne({
    name: !token_result ? note_file.music : token_result.music,
    public: true,
    file_type: 'music',
  })
  if (!music) {
...
app.post('/chat-report', utils.isLogin, async (req, res, next) => {
  ...
  await Chat.updateOne(
    ...
    { reported: true, reported_by: req.user.fullID },
  )
  return res.send('신고가 접수되었습니다. 관리자 확인 후 신고가 처리됩니다.')
...
app.post(
  ...
  async (req, res, next) => {
    ...
    if (!token_result.replay) {
      req.flash(
        'Error',
        '이 파일은 채보 파일입니다. 리플레이 파일을 업로드하세요.',
      )
      return res.redirect('/replay')
    }
