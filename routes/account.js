...
app.get('/verifymail', async (req, res, next) => {
  ...
  if (req.query.join == 'true') {
    await User.updateOne(
      { fullID: token_result.account_fullID },
      { join_finish: true },
    )
    res.send(
      `<h1>요청이 정상적으로 처리되었습니다!</h1><h2><a href="/">메인으로 돌아가기</a></h2>`,
    )
    return
  } else {
    ...
    return res.send(
      `<h1>작업 취소가 완료되었습니다. 이 링크 없이는 다른 사람이 이 이메일 관련 설정을 할 수 없습니다.</h1><h2><a href="/">메인으로 돌아가기</a></h2>`,
    )
  }
})
...
`,
  ...
  req.flash(
    'Info',
    '이메일을 발송했습니다. 이메일로 보내진 링크를 눌러 비밀번호를 변경하세요.(30분간만 유효합니다.)',
  )
  return res.redirect('/find_my_password')
})
...
app.post('/change_password', async (req, res, next) => {
  ...
  await User.updateOne(
    {
      fullID:
        req.body.from_mypage == 'true'
          ? req.user.fullID
          : token_result.account_fullID,
    },
    { password: hash },
  )
  return res.send(
    `<h1>비밀번호가 변경되었습니다.</h1><h2><a href="/">메인으로 돌아가기</a></h2>`,
  )
})
