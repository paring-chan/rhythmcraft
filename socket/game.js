...
module.exports = (io, app) => {
  io.on('connection', async (socket) => {
    const user = await User.findOne({
      fullID: socket.request.session.passport.user.fullID,
    })
    ...
    if (check_user != null) {
      io.to(`user_${user.fullID}`).emit('msg', {
        action: 'exit',
        message:
          '다른 클라이언트에서 이 계정으로 접속하였습니다.\n연결을 해제합니다.',
      })
    ...
    if (
      ...
      room.password != null &&
      room.password != '' &&
      room.password != socket_url_query.password
    ) {
      socket.emit('msg', {
        action: 'exit',
        message: '비밀번호가 틀렸습니다.',
    ...
    let badge
    if (user.equip.image_badge != null)
      badge = await Item.findOne({ product_id: user.equip.image_badge })
    ...
    socket.on('msg', async (data) => {
      ...
      switch (data.action) {
        case 'gamestart':
          ...
          if (checkroom.note != null)
            for (let key in checkroom.note.jscode) {
              if (
                !checkroom.note.jscode[key]
                  .split(' ')
                  .join('')
                  .startsWith('note_speed=note_speed/')
              )
                continue

              const this_note_speed = Number(
                checkroom.note.jscode[key]
                  .split(' ')
                  .join('')
              ...
              if (
                !isNaN(this_note_speed) &&
                this_note_speed > most_slow_note_speed
              )
          ...
          if (before_packet != checkroom.packet_multiplier)
            socket.emit('Chat', {
              nickname: '시스템',
              chattype: 'system',
        ...
        case 'gameready':
          ...
          if (
            checkroom.ready_player >= checkroom.now_player ||
            checkroom.now_player == 1
          ) {
            io.to(`room_${url_query.room}`).emit('msg', {
            ...
            await Room.updateOne(
              { roomcode: url_query.room },
              {
                ready_player: 0,
                starttimestamp:
                  new Date().getTime() + 3000 - checkroom.startpos,
              },
            )

            if (checkroom.room_for_note_test)
              starttimestamp = new Date().getTime() - checkroom.startpos
            else
            ...
            if (checkroom.note != null) {
              ...
              if (checkroom.trusted)
                for (let i in checkroom.note.jscode) {
                  if (Number(i) > checkroom.startpos)
                    rtnote_timeout.push(
                      setTimeout(() => {
                        io.to(`room_${url_query.room}`).emit('msg', {
                          action: 'eval',
              ...
              if (checkroom.note.chat != null && checkroom.trusted)
                for (let i in checkroom.note.chat) {
                  if (Number(i) > checkroom.startpos)
                    rtnote_timeout.push(
                      setTimeout(() => {
                        io.to(`room_${url_query.room}`).emit(
                          'Chat',
                          checkroom['note']['chat'][i],
                        )
                      }, Number(i) / (checkroom.pitch / 100) + countdown - checkroom.startpos / (checkroom.pitch / 100) + checkroom.note_speed),
                    )
                  else
            ...
            io.to(`room_${checkroom.roomcode}`).emit('msg', {
              action: 'eval',
              message: `note_speed = ${
                checkroom.note_speed / checkroom.packet_multiplier
              }`,
            })
            await Room.updateOne(
          ...
          if (master || room.auto_manage_room) {
            ...
            if (checkroom.room_for_note_test) {
              socket.emit('msg', {
                action: 'redirect',
                url: `/editor?name=${
                  checkroom.note_name_for_note_test
                }&startpos=${
                  (new Date().getTime() -
                    starttimestamp -
                    1000 / (checkroom.pitch / 100)) /
                  (checkroom.pitch / 100)
                }&pitch=${checkroom.pitch}&autoplay=${
                  checkroom.autoplay
                }&unsafe=${checkroom.trusted}`,
              })
            }
    ...
    socket.on('SpectateUser', async (data) => {
      ...
      if (!checkroom.note && data.fullID == checkroom.master)
        return socket.emit('msg', {
          action: 'alert',
    ...
    socket.on('ChangeRoomSetting', async (data) => {
      ...
      if (
        room.room_for_note_test ||
        room.room_for_single_play ||
        room.room_for_workshop ||
        room.room_for_replay
      )
        return socket.emit('msg', {
          action: 'alert',
          message: '권한이 없습니다.',
      ...
      if (data.note != 'rhythmcraft_mode') {
        ...
        if (path.extname(note.name) == '.signedrhythmcraft') {
          ...
          if (token_result.error)
            return socket.emit('msg', {
              action: 'alert',
              message: `채보 오류 : ${token_result.message}`,
            })
    ...
    socket.on('GiveNote', async (data) => {
      ...
      if (master && !room.note) {
        ...
        if (key != null) {
          ...
          rtnote['note'][`note${key}`].push(
            (new Date().getTime() - starttimestamp) * (pitch / 100),
          )
        }
      }
    })
    ...
    socket.on('Chat', async (data) => {
      ...
      params.splice(0, 1)
      if (data.chat.startsWith('/') && checkuser.admin)
        switch (data.chat.replace('/', '').split(' ')[0]) {
          case 'debug':
            switch (params[0]) {
              case 'autoplay':
                socket.emit('msg', {
              ...
              case 'notepersecond':
                if (!params[1] || !params[2]) {
                  socket.emit('Chat', {
                    ...
                    chattype: 'system',
                    chat:
                      '옵션이 잘못되었습니다.<br>사용법 : /debug notepersecond <노트번호> <간격 ms>',
                    verified: true,
                  })
                  break
                }
          ...
          case 'packet':
            ...
            if (p_params.length != 3) {
              socket.emit('Chat', {
                ...
                chattype: 'system',
                chat:
                  '파라미터 수가 잘못되었습니다. 소켓 대상, 이벤트, 메시지를 //로 구분하여 입력해주세요.',
                verified: true,
              })
              break
            }
      ...
      if (checkuser.block_chat >= Date.now())
        return socket.emit('Chat', {
          nickname: `시스템`,
          chattype: 'system',
          chat: `관리자에 의해 채팅이 정지되어 ${new Date(
            checkuser.block_chat,
          ).toLocaleDateString()} ${new Date(
            checkuser.block_chat,
          ).toLocaleTimeString()}까지 채팅 사용이 불가능합니다.<br>채팅 정지 사유 : ${
            checkuser.block_chat_reason || '사유가 지정되지 않음'
          }`,
          verified: true,
        })
      ...
      let badge
      if (checkuser.equip.image_badge != null)
        badge = await Item.findOne({ product_id: checkuser.equip.image_badge })
      io.to(`room_${url_query.room}`).emit('Chat', {
    ...
    socket.on('SaveToLibrary', async (data) => {
      ...
      return socket.emit('msg', {
        action: 'alert',
        message: `채보 ${data.filename}이(가) 성공적으로 업로드되었습니다.`,
      })
    })
    ...
    socket.on('Invite', async (data) => {
      if (!user.friends.includes(data.user))
        return socket.emit('msg', {
          action: 'alert',
          message: '해당 유저는 친구가 아닙니다.',
        })

      const check_roomuser = await RoomUser.findOne({
        fullID: data.user,
        roomcode: url_query.room,
      })
      if (check_roomuser != null)
        return socket.emit('msg', {
          action: 'alert',
      ...
      app
        ...
        .emit('toast', {
          ...
          title: '게임 초대',
          text:
            `${utils.escapeHTML(user.nickname)}님이 "${utils.escapeHTML(
              checkroom.name,
            )}" 게임에 초대하였습니다.<br>아래 버튼으로 참가하세요.<br>` +
            `<a href="/game?room=${checkroom.roomcode}#pwu=${encodeURIComponent(
              checkroom.password || 'nopassword',
            )}" class="btn btn-primary">참가</a>`,
          options: {
            delay: 10000,
          },
          allow_html: true,
    ...
    socket.on('disconnect', async () => {
      ...
      if (master) {
        ...
        let checkroom = await Room.findOne({ roomcode: url_query.room })
        app.get('socket_main').emit('msg', { action: 'reload_room' })
        io.to(`room_${url_query.room}`).emit('userLeave', {
          nickname: user.nickname,
          fullID: user.fullID,
        })
        io.to(`room_${url_query.room}`).emit('Chat', {
