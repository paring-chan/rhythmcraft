...
window.onload = () => {
  ...
  document.getElementById('Play').onclick = function () {
    ...
    if (result == 'ok')
      location.href = `/testnote?note=${note_name}&startpos=${note_time}&fromeditor=true&pitch=${pitch}&autoplay=${autoplay}&unsafe=${unsafe}`
  ...
  Array.from(document.getElementsByClassName('note_area')).forEach((ele) => {
    ele.onclick = function (e) {
      ...
      note['note'][`note${notenum}`].push(
        Math.round(getms(innerHeight - e.clientY - 24.5)) + note_time,
      )
      renderNote(note, note_time)
    ...
    ele.oncontextmenu = function (e) {
      ...
      note['jscode'][
        Math.round(getms(innerHeight - e.clientY - 24.5)) + note_time + 30
      ] = ''
      renderNote(note, note_time)
  ...
  document.getElementById('SaveScript').onclick = function () {
    ...
    note['jscode'][inputscript.dataset.effect] = inputscript.value
      .split('\n')
      .join('/*nextline*/')
    note['jscode'][inputscript.dataset.effect] = note['jscode'][
      inputscript.dataset.effect
    ]
      .split('`')
      .join('/*grave*/')
    note['jscode'][inputscript.dataset.effect] = note['jscode'][
  ...
  document.getElementById('help_button').onclick = function () {
    document.getElementById('help_area').hidden = !document.getElementById(
      'help_area',
    ).hidden
  }
...
function renderNote(note, look_time) {
  ...
  for (let i in note.note) {
    note['note'][i].forEach((time) => {
      const position =
        ((innerHeight * 0.65) / note_speed) * (time - look_time) +
  ...
  for (let i in note.jscode) {
    const position =
      ((innerHeight * 0.65) / note_speed) * (Number(i) - look_time) +
      (innerHeight * 0.65) / note_speed +
      innerHeight * 0.3
    if (position < innerHeight && position > 0) {
      ...
      newscript.oncontextmenu = function (e) {
        e.preventDefault()
        Array.from(document.getElementsByClassName('editscript')).forEach(
          (ele) => {
            ele.hidden = false
          },
        )
        ...
        inputscript.value = inputscript.value
          .split('/*closebracket*/')
          .join('}')
...
function getms(px) {
  return (
    (note_speed * px - 0.3 * innerHeight * note_speed) / (0.65 * innerHeight) -
    1
  )
}
